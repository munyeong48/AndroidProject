...
public class GoogleAnalyticsBuilder {
    ...
    public GoogleAnalyticsBuilder(Activity activity) {
...
//                        URL = URLDecoder.decode(URL, "UTF-8");
    ...
    public void GADataSend_Timing(Map<String, String> GAInfo) {
        try {
            if(mTracker != null) {
                ...
                while (sIterator.hasNext()) {
                    ...
                    if (GAInfo.get(key) != null && GAInfo.get(key).length() > 0) {
                        ...
                        if (key.toLowerCase().equals("timingvalue")) {
                            if (NumericChecker(GAInfo.get(key)))
                                timingBuilder.setValue(Long.valueOf(GAInfo.get(key)));
                        }
                ...
                if (NumericChecker(GAInfo.get("timingvalue")))
                ...
                while (sIterator.hasNext()) {
                    ...
                    if (GAInfo.get(key) != null && GAInfo.get(key).length() > 0) {
                        ...
                        if (key.toLowerCase().equals("fatal")) {
                            if (GAInfo.get("fatal").equals("true") || GAInfo.get("fatal").equals("Y"))
                                exceptionBuilder.setFatal(true);
                            else if (GAInfo.get("fatal").equals("false") || GAInfo.get("fatal").equals("N"))
    ...
    public static boolean NumericChecker(String s) {
        try {
    ...
    public static class WebAppInterface {
                ...
                if(mTracker != null) {
                    ...
                    if (data.has("ecommerce")) {
                        ...
                        if (ecommerce_data.has("products")) {
                            ...
                            List paa = ConvertJsonArray(product_array);

                            for (int i = 0; i < paa.size(); i++) {

                                HashMap<String, String> product_Hashmap = (HashMap<String, String>) paa.get(i);
                                ...
                                while (keys.hasNext()) {
                                    ...
                                    if (key.contains("category")) item.setCategory(product_Hashmap.get(key));
                                    if (key.contains("price")) item.setPrice(Double.parseDouble(product_Hashmap.get(key)));
                                    if (key.contains("quantity")) item.setQuantity(ConvertStringObjectData(product_Hashmap.get(key)));
                                    if (key.contains("variant")) item.setVariant(product_Hashmap.get(key));
                                    if (key.contains("coupon")) item.setCouponCode(product_Hashmap.get(key));
                                    if (key.contains("position")) item.setQuantity(ConvertStringObjectData(product_Hashmap.get(key)));
                                    ...
                                    if (key.contains("metric")) {
                                        int Number = Integer.parseInt(key.replaceAll("[^0-9]", ""));
                                        item.setCustomMetric(Number, ConvertStringObjectData(product_Hashmap.get(key)));
                                    }
                                }
                                if (!obj_ecommerce.has("impressions")) {
                        ...
                        if (productAction != null) {
                            if (sType.equals("P"))
                                Screenview_Builder.setProductAction(productAction);
        ...
        @JavascriptInterface
        public void FA_DATA(String JsonData) {

        }

        private Integer ConvertStringObjectData(Object data){
